...
def updateDB(ranges,cursor):
    for range in ranges:
        query = "UPDATE b73v4ranges SET type='exon' WHERE chr="+str(range[2])+" AND pos >= "+str(range[0])+" AND pos <= "+str(range[1])+";"
        print query
        #cursor.execute(query)


def getModels(cursor,limit,offset,chrom):
    query = "select chr,pos,type,ends from b73v4ranges WHERE chr="+str(chrom)+" ORDER BY chr,pos LIMIT "+str(limit)+" OFFSET "+str(offset)+";"
    cursor.execute(query)
    #print "loaded models."
