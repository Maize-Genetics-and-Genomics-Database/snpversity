// Check valid submission (file input only for now)
...
function estimate(){
    // TODO: 1. Pass Taxa array, any uploaded file
    //       2. Display loading icon
    //if (validateForm()){
        $('#errorMsg').text("Estimating time...").show();
        $.ajax({
            type: "POST",
            data: {
                action: "getTime",
                assembly: $("#assembly").val(),
                range: $("#endPosition").val() - $("#startPosition").val(),
                taxa: $("#taxa").serialize(),
                dataset: $("#dataSet").val(),
                stockFile: $("#stockFile").val()
            },
            url: "get_time_estimate.php",
            success: function (data) {
                console.log(data);
                var time_obj = JSON.parse(data);
                $('#errorMsg').text("Time estimate of query using " + time_obj.stocks + " stock(s) and " + time_obj.range + " bp's is: " + time_obj.time + "s.").show();
            }
        });
   // }
}
