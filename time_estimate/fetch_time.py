...
def plot(dataset, model_type='knn', point_selection='random'):
    ...
    ax.scatter(taxa,ranges,times,c='r', marker='o')
    ax.scatter(stocks_generated,ranges_generated,time_predictions,c='b',marker='^')
    plt.show()
...
def main():
    ...
    if len(sys.argv) == 4:
        ...
        if dataset == 'all':
            ...
            print '---------SUMMARY---------'
            print '{2} of {0}: {1}'.format(dataset,mse,VALIDATION_TYPE)
            plot(dataset, MODEL_TYPE)
    else:
